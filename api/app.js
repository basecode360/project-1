  ...
  try {
    ...
    if (tokenManager.isTokenValid()) {
      return res.json({
        ...
        expires_in_seconds: Math.floor((tokenManager.tokens.expiryTime - new Date()) / 1000)
      });
    }
...
app.get('/auth/refresh', async (req, res) => {
  if (!tokenManager.tokens.refreshToken) {
    return res.status(400).json({
      success: false,
      message: 'No refresh token available. Please complete the authorization flow first.',
      auth_url: '/auth/login'
    });
  }
