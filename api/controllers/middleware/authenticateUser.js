...
export async function generateEbayAuthCode(options = {}) {
  ...
  try {
    ...
    if (error.message.includes('timeout')) {
      throw new Error('eBay authorization timed out - possibly due to 2FA or user inaction');
    }
    if (error.message.includes('Execution context was destroyed')) {
      throw new Error('Page navigated too quickly before code could be extracted');
    }
    throw error;
  } finally {
    await browser.close();
