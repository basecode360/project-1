...
const getItemPriceHistory = async (req, res) => {
  try {
    ...
    if (priceHistory.length > 0) {
      const firstSuccessful = [...priceHistory].reverse().find(p => p.success);
      ...
      if (firstSuccessful && lastSuccessful) {
        ...
        const percentChange = firstPrice > 0 ? (totalChange / firstPrice) * 100 : null;
        ...
        stats = {
          ...
          priceDirection: 
            totalChange > 0 ? 'increased' : 
            totalChange < 0 ? 'decreased' : 'unchanged',
...
/**
 * NEW: Add a manual price history record
 */
const addManualPriceRecord = async (req, res) => {
  try {
    const { itemId, sku, price, oldPrice, currency = 'USD', title, notes } = req.body;
    ...
    // Calculate price change if old price provided
