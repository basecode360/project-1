...
const pricingStrategySchema = new mongoose.Schema({
  ...
  beatBy: {
    ...
    // Required only for BEAT_LOWEST
    validate: {
      validator: function(v) {
        return this.repricingRule !== 'BEAT_LOWEST' || v;
      },
  ...
  stayAboveBy: {
    ...
    // Required only for STAY_ABOVE
    validate: {
      validator: function(v) {
        return this.repricingRule !== 'STAY_ABOVE' || v;
      },
  ...
  value: {
    ...
    validate: {
      validator: function(v) {
        ...
        if (this.repricingRule === 'BEAT_LOWEST' || this.repricingRule === 'STAY_ABOVE') {
    ...
    validate: {
      validator: function(v) {
        if ((this.beatBy === 'PERCENTAGE' || this.stayAboveBy === 'PERCENTAGE') && v > 1) {
          return false;
        }
        return true;
      },
      message: 'Percentage values should be in decimal format (e.g., 0.10 for 10%)'
    }
  },
...
pricingStrategySchema.pre('validate', function(next) {
  if (this.repricingRule === 'BEAT_LOWEST' && (!this.beatBy || this.value === undefined)) {
    return next(new Error('BEAT_LOWEST strategy requires beatBy and value fields'));
  }
  
  if (this.repricingRule === 'STAY_ABOVE' && (!this.stayAboveBy || this.value === undefined)) {
    return next(new Error('STAY_ABOVE strategy requires stayAboveBy and value fields'));
  }
